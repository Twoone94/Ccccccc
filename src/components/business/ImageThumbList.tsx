...
export default function ImageThumbList({
  setContainerRef,
  options,
  bar,
  setBarActive,
      ...
      >
        <div className={hasMany ? 'flex flex-col items-center gap-2 py-1 w-full px-1' : 'w-full px-1'}>
          {options.filter(v => v !== null).map((v, idx) => (
            <div key={idx} className={'rounded mx-auto cursor-pointer'} style={{ width: thumbWidth ? `${thumbWidth}px` : '100%', maxWidth: 'calc(100% - 8px)' }} onClick={() => onSelect?.(v ?? null, idx)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onSelect?.(v ?? null, idx) }}>
              <div className="bg-black rounded" style={{ aspectRatio: '16 / 9', overflow: 'hidden' }}>
                {v && (<img src={v} alt="分镜候选" className="w-full h-full object-cover" />)}
              </div>
            </div>
          ))}
        </div>
